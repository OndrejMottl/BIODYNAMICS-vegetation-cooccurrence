---
date: 2025/05/15
date-format: long
date-modified: last-modified
---

# BIODYNAMICS — Vegetation Co-occurrence

This project analyses **co-occurrence patterns of plant taxa across space and time** using paleoecological pollen records and co-located palaeoclimate reconstructions. The central aim is to quantify how much of the observed co-occurrence signal is explained by shared abiotic (environmental) responses versus genuine biotic associations — and whether those patterns are consistent across geographic scales and through the last 20,000 years.

The project is part of the [BIODYNAMICS](https://bit.ly/BIODYNAMICS) initiative.

::: {.callout-tip}
## Explore the project

- [Methods & Overview](Documentation/Website/about.qmd) — data sources, two-pipeline design (temporal and spatial), all pipeline stages, and analysis configurations
- [Reproducibility](Documentation/Website/installation.qmd) — how to install dependencies and run the pipeline
- [Documentation](Documentation/Website/Documentation/documentation.qmd) — individual function reference and code coverage
:::

