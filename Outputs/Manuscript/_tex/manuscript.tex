...
We sourced all vegetation community and climate records from
\textbf{VegVault} (version 1.0.0; {[}cite{]}), a harmonised SQLite
database that integrates fossil pollen archives and contemporary
vegetation datasets with co-registered palaeoclimate reconstructions and
plant functional traits. Sourcing all three data types required by this
study --- fossil and contemporary vegetation community records,
...
{[}cite{]}, and contemporary records are drawn from modern vegetation
databsses BIEN {[}cite{]} and sPlotOpen {[}cite{]}. Plant functional
traits were assembled in VegVault from
TRY\textsuperscript{\textbf{Kattge2020?}} and additional contributed
datasets, with the original dataset and data-source identities retained
for provenance and correction. Palaeoclimate predictors are
...
fossil pollen and contemporary sites per 500-year time slice and per
continental region) is summarised in Supplementary Figure S{[}TODO{]}.
...
target rank across all morphotypes. For each morphotype, we retrieved
taxonomy with \texttt{taxospace::get\_classification()} against the GBIF
taxonomic backbone {[}cite{]}, using non-exact matching where needed to
resolve legacy or region-specific names while preserving traceability of
the returned identifiers. Where the morphotype was too broadly defined
morphologically to support a confident genus assignment --- typically
for compound types that aggregate morphologically similar pollen from
several genera --- the record was retained at family rank rather than
discarded, preserving ecological information that would otherwise be
lost entirely. We excluded non-plant records (e.g., fungal spores and
fern spores flagged in source databases). We supplemented the backbone
matching with a curated project-specific correction table that records
manually validated taxonomic decisions for morphotypes known to be
mis-classified or absent from the backbone, ensuring that recurring edge
cases are resolved consistently across all analyses. All classifications
were reviewed and validated by expert botanists to catch systematic
errors arising from regional nomenclatural traditions or legacy
morphotype names not covered by the backbone. Morphotypes that remained
unresolved after automated matching and manual correction were exported
as explicit missing-taxon reports and tracked separately from the final
harmonised tables. Because the taxa present in any given analysis depend
on the geographic extent and temporal window queried, we compiled a
separate harmonisation table for each continental region rather than a
single global table. We provide these tables as Supplementary Tables
S{[}TODO{]} (one per continental region).
...
avoid artefacts introduced by arbitrary bin edges. Temporal
interpolation was applied only to fossil sequences; contemporary samples
were retained as present-day observations at age 0. We excluded taxa
...
co-occurrence {[}TODO: cite{]}. Before model assembly, we restricted
analyses to the intersection of site-age samples present in community,
coordinate, and abiotic tables, and required abiotic records to be
complete across all retained abiotic variables. Finally, to ensure
sufficient data density for reliable model estimates, we excluded
spatial units with too few sites or too few qualifying taxa after
filtering. We calibrated the exact thresholds separately for each
spatial unit, because the number of sites and taxa varies across units
and the optimal thresholds depend on the data structure of each unit.
For contemporary vegetation records, we applied an additional
preprocessing stage before shared filtering: we generated a
quality-control report to flag impossible values (e.g., non-zero modern
ages, invalid coordinates, and missing or negative abundance values),
removed exact duplicate records deterministically, and aggregated
colocated records only within eligible same-prefix source groups at
identical coordinates and age. Cross-database colocations (notably
BIEN/sPlot overlaps) were retained unchanged and tracked separately for
auditability. For aggregated colocations, we replaced original records
with synthetic site identifiers and propagated aggregation consistently
across community, abiotic, and coordinate tables via explicit mapping
tables.
...
compared across predictors with different natural units. Rows with
missing values in the abiotic predictor table were removed prior to
scaling, so all scaled predictors supplied to the model were
complete-case. In analyses with age as a predictor, we centred age but
did not variance-scale it, so that interaction coefficients retain their
natural interpretation in units of years. We projected site coordinates
to a metric coordinate system (ETRS89-LAEA, EPSG:3035) so that
geographic distances are measured in kilometres rather than decimal
degrees.
...
distance matrix among sites. For spatiotemporal analyses, we computed
MEMs on a sample-level three-dimensional coordinate matrix (x, y, age),
where age was converted from years to kiloyears and each axis was
z-scored before eigenvector extraction so that spatial and temporal
extents contributed on comparable scales. In both spatial and
spatiotemporal settings, if the requested number of MEM predictors
exceeded the number of positive eigenvectors returned by the
decomposition, we clamped retention to the available positive set.
...
\item
  \textbf{B} --- variance attributable to the residual biotic covariance
  (co-occurrence patterns unexplained by environment or space,
  interpreted as a signal of potential biotic
...
ecological interpretation. For all temporal summaries and cross-model
comparisons, we then re-expressed the seven-fraction output as three
unique components (Abiotic, Spatial, Associations) using a Shapley
equal-split allocation of shared fractions (AB, AS, BS, ABS) among their
contributing components. Concretely, AB and AS are split evenly between
their two parent components and ABS is split one-third to each of A, S,
and B, yielding adjusted unique contributions that sum to total
explained variance. We finally converted these adjusted components to
percentages of explained variance within each fitted model, and these
percentage trajectories are the quantities reported in temporal
comparisons.
...
at the continental scale, fewer than 10 at the regional scale, or fewer
than 5 at the local scale, reflecting the reduced number of sites per
unit at finer grains. We excluded a spatial unit entirely if fewer than
5 qualifying taxa remained after all pre-processing at the continental
scale; the corresponding lower bounds were 4 taxa at the regional scale
and 3 taxa at the local scale.
...
co-occurrence network metrics (connectance, nestedness, and modularity)
directly from the binarised community matrix at each time slice to
characterise the structural properties of the co-occurrence network
independently of any model assumptions. We then aggregated results from
successive time slices into temporal trajectories of variance fractions
and network metrics, revealing how the relative importance of
environment, space, and biotic residuals --- and the structure of the
co-occurrence network itself --- has evolved over geological time. We
ran analyses for three continental regions: Europe, North America, and
northern Asia.
...
\textbf{Trait data, correction and audit.} Plant functional traits
...
taxonomic boundaries. Trait processing followed seven ordered steps.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Extraction and provenance.} We extracted six continuous trait
  domains from VegVault 1.0.0: diaspore mass, leaf area, leaf mass per
  area, leaf nitrogen content per unit mass, plant height, and stem
  specific density (see Supplementary Table S{[}TODO{]} for definitions
  and units). The extraction retained dataset, data-source, sample,
  trait, and taxon identifiers so that every correction could be
  restricted to its documented source and audited at record level. We
  removed records without a taxon identifier or with a non-finite trait
  value.
\item
  \textbf{Source-level unit harmonisation.} Starting from the otherwise
  unchanged extraction, we applied a version-guarded compatibility layer
  before any taxon-level review. The 10 approved rules corrected 28,856
  records by multiplying affected leaf-area and plant-height sources by
  100 and the affected stem-density source by 0.001. Each rule specified
  the VegVault version, source identifier and description, trait domain,
  factor, and expected match count. The pipeline stopped if the database
  version, source identity, selector, or match count differed from the
  approved contract. Record-level audit tables retained the original
  value, corrected value, factor, and source identifiers.
\item
  \textbf{Raw review-candidate detection.} We constructed one candidate
  for each taxon--trait-domain group containing a non-positive value, a
  within-taxon outlier, or a key preserved from the archived trait
  review. A within-taxon outlier was a value whose absolute deviation
  from the group median exceeded 1.5 times the interquartile range; this
  test was applied only to groups with at least 10 records and a
  non-zero interquartile range. We also calculated a whole-domain
  diagnostic fence of 3 interquartile ranges to identify possible
  source-wide problems. Whole-domain flags informed source investigation
  but did not create taxon-level review candidates by themselves.
\item
  \textbf{Raw evidence review and correction.} Each candidate received
  an atomic decision of no action, exclusion, or multiplication by a
  positive factor. Correction decisions could be restricted by exact
  trait name, dataset, and inclusive or exclusive value bounds. Evidence
  came from the archived review, deterministic distributional summaries,
  grouped-source comparisons, and bounded AI-assisted evidence
  gathering. AI-generated recommendations remained proposals; only
  decisions explicitly approved by the project owner with reviewer and
  date metadata entered the pipeline. The completed raw gate contained
  4,339 approved decisions: 4,262 no-action decisions, 20 exclusions,
  and 57 residual scaling rules. Before applying these decisions,
  validation rejected incomplete coverage, unmatched selectors,
  overlapping or conflicting rules, and any taxon-level scaling that
  would duplicate or contradict source-level scaling.
\item
  \textbf{Taxonomic resolution and second review.} After applying raw
  decisions, we classified trait taxa against the same taxonomic
  backbone and curated auxiliary table used for community records, then
  resolved each trait name to its finest supported rank. We rebuilt the
  candidate set under the resolved names because merging synonymous or
  differently resolved names can reveal distributions that were not
  apparent before harmonisation. The classified gate contained 4,223
  approved no-action decisions, confirming that no additional correction
  was required after source scaling, raw review, and taxonomic
  resolution.
\item
  \textbf{Matching and aggregation.} For each continental
  functional-type analysis, we matched corrected trait records to the
  canonical taxonomic lineage of each community taxon. A community taxon
  resolved above species level therefore received observations from all
  matching descendant trait taxa. We calculated the median for each
  community-taxon--trait-domain combination and pivoted the result to a
  taxon × trait matrix.
\item
  \textbf{Interim release and uncertainty handling.} We froze the
  resulting records as an interim analysis release specific to VegVault
  1.0.0. A future VegVault version intentionally fails this
  compatibility contract until the local source rules are explicitly
  retired or replaced. All six traits were retained in the primary
  analysis, but leaf mass per area and leaf nitrogen content per unit
  mass were marked as having provisional unit provenance, and stem
  specific density was marked as retaining interim source uncertainty.
  Leaf mass per area received no scaling or exclusion because the
  available evidence did not justify a correction, rather than because
  its source units were considered resolved. We therefore defined four
  separately stored sensitivity classifications that omitted leaf mass
  per area, leaf nitrogen content, both provisional traits together, or
...
classification. Before calculating distances, we log10-transformed
diaspore mass, leaf area, leaf mass per area, and plant height to reduce
the influence of their strongly right-skewed measurement scales; leaf
nitrogen content and stem specific density were retained on their
original scales. We quantified pairwise dissimilarities among taxa using
the Gower distance metric, which range-standardises each trait and
calculates each taxon pair from their jointly observed traits without
requiring imputation. Pairs sharing no observed trait received the
maximum dissimilarity of one rather than a non-finite distance. We then
grouped taxa by Ward's minimum-variance hierarchical clustering applied
to the Gower distance matrix. We chose Ward's method because it
minimises within-cluster total variance at each merge step, producing
compact, well-separated clusters that are more likely to correspond to
ecologically coherent functional strategies than linkage methods that
optimise only local cohesion.
...
The analytical codebase comprises 364 modular R functions, each in its
...
and visualisation. Each function has a dedicated unit-test file (399
test files in total), and the test suite achieves 96 \% line coverage
across all function files as measured by \{covr\} {[}TODO: cite{]}. All
364 functions are documented and are publicly accessible through a
...
\protect\phantomsection\label{cell-fig-cooccurrence-overview}
\textsubscript{Source:
\href{https://OndrejMottl.github.io/BIODYNAMICS-vegetation-cooccurrence/manuscript.qmd.html}{Article
Notebook}}
...
\protect\phantomsection\label{cell-fig-s1}
\textsubscript{Source:
\href{https://OndrejMottl.github.io/BIODYNAMICS-vegetation-cooccurrence/manuscript.qmd.html}{Article
Notebook}}
