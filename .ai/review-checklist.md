...
Canonical checklist for reviewing changes in this repository. This checklist is extracted from the `changes-reviewer` agent workflow and is intended for humans and assistants that need a compact validation target.
...
3. Read the changed files and review behavior, style, documentation, tests, and workflow safety.
...
- R source lines stay within the project line-length rule.
...
- No use of superseded `dplyr::transmute()`; prefer `dplyr::mutate()` with `dplyr::select()` where needed.
...
- Functions in `R/Functions/` have roxygen2 documentation with `@param`, `@return`, and `@examples`.
...
- Markdown and GitHub prose is not hard-wrapped at 80 characters or any other fixed width.
- Each `.md` paragraph and list item stays on one source line unless Markdown structure or an intentional hard break requires another line.
- Quarto prose follows `.ai/quarto.md`: one complete sentence per source line, with no fixed-width wrapping.
