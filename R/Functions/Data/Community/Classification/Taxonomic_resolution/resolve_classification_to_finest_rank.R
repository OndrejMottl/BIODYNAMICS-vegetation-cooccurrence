...
#' `build_classification_table()`) containing a `sel_name` column
...
#' column named by `vec_taxon_column_name` (character, default
...
#' `sel_name` and the column named by `vec_taxon_column_name`.
...
resolve_classification_to_finest_rank <- function(
  ...
  assertthat::assert_that(
    base::is.character(vec_taxon_column_name),
    base::length(vec_taxon_column_name) == 1L,
    base::nchar(vec_taxon_column_name) > 0L,
    msg = base::paste0(
      "`vec_taxon_column_name` must be a single non-empty character ",
      "string."
    )
  )
  ...
  res_classification_resolved <-
    ...
    tidyr::pivot_longer(
      ...
      names_to = "rank",
      values_to = vec_taxon_column_name
    ) |>
    dplyr::filter(
      !base::is.na(.data[[vec_taxon_column_name]])
    ) |>
    dplyr::left_join(
      data_rank_order,
    ...
    dplyr::select(
      dplyr::all_of(
        base::c("sel_name", vec_taxon_column_name)
      )
    )

  return(res_classification_resolved)
}
