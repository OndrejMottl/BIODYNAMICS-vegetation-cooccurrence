#' @title Prepare a classification table for community taxa
#' @description
#' Re-keys a classification table so that `sel_name` equals the
#' classified taxon name at the specified `vec_taxonomic_resolution`,
...
#' Rank columns finer than `vec_taxonomic_resolution` are set to
...
#' `vec_taxonomic_resolution` are used to coalesce a `classified_name`
...
#' @export
prepare_classification_table_for_community_taxa <- function(
  ...
  assertthat::assert_that(
    base::is.character(vec_taxonomic_resolution) &&
      base::length(vec_taxonomic_resolution) == 1L,
    msg = stringr::str_c(
  ...
  return(res_classification_table_prepared)
}
