#' @title Select unclassified taxa
#' @description
...
#' @export
select_unclassified_taxa <- function(
    vec_community_taxa,
    data_classification_table) {
  assertthat::assert_that(
    base::is.character(vec_community_taxa),
    base::length(vec_community_taxa) > 0,
  ...
  assertthat::assert_that(
    "sel_name" %in% base::names(data_classification_table),
    msg = "data_classification_table should contain a 'sel_name' column"
  )

  assertthat::assert_that(
    !base::any(
      base::duplicated(data_classification_table[["sel_name"]])
    ),
    msg = stringr::str_c(
      "data_classification_table should not contain duplicate ",
      "'sel_name' values"
    )
  )
  ...
  res_unclassified_taxa <-
    dplyr::anti_join(
      base::data.frame(taxon = vec_community_taxa) |>
