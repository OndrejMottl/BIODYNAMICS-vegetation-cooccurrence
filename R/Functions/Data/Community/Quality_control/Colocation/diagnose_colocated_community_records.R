#' @title Diagnose Colocated Community Records
#' @description
#' Returns same-coordinate, same-age community colocations with source-prefix
...
#' @export
diagnose_colocated_community_records <- function(
    data_community = NULL,
  ...
  data_record_coordinates <-
    ...
    dplyr::mutate(
      source_prefix = classify_dataset_prefix(
        vec_dataset_names = dataset_name
      ),
      record_key = stringr::str_glue("{dataset_name}::{sample_name}")
    )

  data_records_missing_coordinates <-
    data_record_coordinates |>
    dplyr::filter(
  ...
  if (
    base::nrow(data_records_missing_coordinates) > 0L
  ) {
    cli::cli_abort(
      c(
        ...
        "i" = stringr::str_glue(
          ...
          "{base::nrow(data_records_missing_coordinates)} ",
          "sample record(s)."
        )
      )
    )
  ...
  return(res_colocation_diagnostics)
}
