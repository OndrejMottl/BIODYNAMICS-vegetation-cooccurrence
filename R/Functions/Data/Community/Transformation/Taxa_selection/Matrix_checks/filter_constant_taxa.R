...
filter_constant_taxa <- function(
  ...
  vec_taxon_standard_deviations <-
    purrr::map_dbl(
      .x = base::colnames(data_community_matrix),
      .f = ~ stats::sd(data_community_matrix[, .x])
    )

  vec_variable_taxa <-
    vec_taxon_standard_deviations > 0

  res_community_matrix_filtered <-
  ...
  return(res_community_matrix_filtered)
}
