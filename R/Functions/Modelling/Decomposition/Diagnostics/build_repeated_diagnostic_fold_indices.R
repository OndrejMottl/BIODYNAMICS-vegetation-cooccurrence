...
#' decomposition diagnostics with [run_decomposition_diagnostic_folds()].
#' @param n_samples
...
#' )
#' @export
build_repeated_diagnostic_fold_indices <- function(
    n_samples,
    n_folds = 2L,
    n_repeats = 1L,
    seed = 900723L) {
  ...
  res <-
    ...
    purrr::map(
      .f = ~ {
        ...
        vec_indices |>
          ...
          rlang::set_names(
            nm = stringr::str_glue(
              "fold_{stringr::str_pad(base::seq_len(n_folds), 3, pad = '0')}"
            )
          )
      }
    ) |>
    rlang::set_names(
      nm = stringr::str_glue(
        "repeat_{stringr::str_pad(base::seq_len(n_repeats), 3, pad = '0')}"
      )
    )
