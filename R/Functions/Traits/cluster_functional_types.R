...
#' `number_of_ft_groups`. The number of groups is expected to have
#' been chosen upstream (e.g. via `select_ft_groups_by_silhouette()`).
#' @param data
...
#' greater than `number_of_ft_groups`.
#' @param dist_mat
...
#' `number_of_ft_groups` via `cli::cli_inform()`.
#' @return
...
#'   \item{functional_type}{Integer label (1..number_of_ft_groups)
#'     giving the functional-type cluster assignment for each
#'     taxon.}
#'   \item{silhouette_width}{Per-taxon silhouette width for
#'     the chosen number_of_ft_groups solution.}
#' }
#' @details
#' **Algorithm**:
#' \enumerate{
#'   \item Cut the dendrogram at `number_of_ft_groups`
#'     via `stats::cutree()`.
...
#' If `nrow(data) < 4` or `number_of_ft_groups >= nrow(data)`,
...
cluster_functional_types <- function(
  ...
  assertthat::assert_that(
    (
      base::is.numeric(number_of_ft_groups) ||
        base::is.integer(number_of_ft_groups)
    ) &&
      base::length(number_of_ft_groups) == 1L &&
      number_of_ft_groups >= 2L,
    msg = "'number_of_ft_groups' must be a single integer >= 2."
  )
  ...
  assertthat::assert_that(
    base::as.integer(number_of_ft_groups) < n_taxa,
    msg = stringr::str_glue(
      "'number_of_ft_groups' ({number_of_ft_groups}) must be",
      " less than nrow(data) ({n_taxa})."
    )
  )

  ft_groups_value <-
    base::as.integer(number_of_ft_groups)

  # Cluster assignment and silhouette widths -----
  vec_labels <-
    stats::cutree(hclust_obj, k = ft_groups_value)
  ...
  if (
    ...
    cli::cli_inform(
      base::c(
        "i" = stringr::str_glue(
          "FT clustering: applying number_of_ft_groups = {ft_groups_value}"
        ),
        "i" = stringr::str_glue(
