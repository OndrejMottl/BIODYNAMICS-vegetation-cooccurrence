...
testthat::test_that(
  "extract_age_uncertainty_from_vegvault returns empty tibble on no matches",
  {
    fake_plan <-
      base::list()
...
# Integration test (requires VegVault database)

testthat::test_that(
  "extract_age_uncertainty_from_vegvault works with VegVault",
  {
    testthat::skip_if(
      base::Sys.getenv("RUN_VEGVAULT_INTEGRATION") != "true",
      "Set RUN_VEGVAULT_INTEGRATION=true to run VegVault integration tests"
    )
