...
Issue #153 moved temporal data functions into a lifecycle-based data
hierarchy. The `Ages/` tests cover extracting sample ages from an in-memory
...
For focused development, source `R/___setup_project___.R` and pass `Ages/` to
`testthat::test_dir()`.
