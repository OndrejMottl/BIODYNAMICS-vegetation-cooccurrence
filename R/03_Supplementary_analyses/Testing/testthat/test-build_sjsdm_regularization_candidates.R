testthat::test_that(
  "build_sjsdm_regularization_candidates() is deterministic",
  {
    data_candidates <-
...
testthat::test_that(
  "build_sjsdm_regularization_candidates() validates configured ranges",
  {
    testthat::expect_error(
      build_sjsdm_regularization_candidates(alpha_cov = base::c(0, 1.1)),
      "alpha_cov"
    )
    testthat::expect_error(
      build_sjsdm_regularization_candidates(lambda_cov = base::c(0, -0.1)),
      "lambda_cov"
    )
    testthat::expect_error(
      build_sjsdm_regularization_candidates(lambda_cov = base::c(0, 0)),
      "lambda_cov"
    )
  }
)
