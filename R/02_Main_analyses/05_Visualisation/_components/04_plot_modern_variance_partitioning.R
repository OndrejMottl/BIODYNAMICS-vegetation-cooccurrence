...
#
#          Visualise modern variance partitioning
#
#                       O. Mottl
#                         2026
#
...
#   spread, and unit-level waffle figures from the latest modern
#   spatial unit table.
# Workflow contract:
#   Requires the dated modern unit table produced by modern synthesis.
#   The latest filename date is selected automatically; confirm it belongs to
#   the intended rerun before publication. No target stores are read here.
#   Produces a variance/spread figure and a separate Associations waffle PDF;
#   both fixed output paths are replaced on rerun.
...
plot_stack <-
  plot_spatial_variance_stack(
    ...
    plot_title = "Modern variance partitioning",
...
fig_modern_waffle <-
  plot_spatial_variance_waffle(
    ...
    plot_title = "Modern mixed variance composition across scales",
    vec_continent_shapes = vec_continent_shapes
  ) +
  ggview::canvas(
    width = graphical_options[["width"]],
...
file_modern_variance <-
  base::file.path(
    path_output_figures,
    stringr::str_glue("modern_variance_partitioning_{tag_date}.pdf")
  )
...
file_modern_waffle <-
  base::file.path(
    path_output_figures,
    stringr::str_glue(
      "modern_variance_partitioning_waffle_{tag_date}.pdf"
    )
  )

ggview::save_ggplot(
  plot = fig_modern_variance,
  file = file_modern_variance,
  width = graphical_options[["width"]],
  height = graphical_options[["height"]],
  units = graphical_options[["units"]],
  dpi = graphical_options[["dpi"]],
...
ggview::save_ggplot(
  plot = fig_modern_waffle,
  file = file_modern_waffle,
  width = graphical_options[["width"]],
  height = graphical_options[["height"]],
  units = graphical_options[["units"]],
  dpi = graphical_options[["dpi"]],
...
base::message("Saved modern variance figure: ", file_modern_variance)
base::message("Saved modern variance waffle: ", file_modern_waffle)
