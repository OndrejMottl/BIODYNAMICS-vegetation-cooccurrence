...
#
#       Slide 08 spatial association spread figure
#
#----------------------------------------------------------#
...
data_paleo_unit <-
  ...
  dplyr::filter(
    .data$resolution_id == selected_resolution_id
  ) |>
  dplyr::mutate(
...
figure_spatial_association_tiles <-
  cowplot::plot_grid(
    plot_association_spread_no_legend,
    cowplot::plot_grid(
      plot_shape_legend,
...
ggview::save_ggplot(
  plot = figure_spatial_association_tiles,
  file = base::file.path(
    path_output,
    "slide_08_spatial_tiles.png"
  ),
  device = ragg::agg_png
)
