...
```{mermaid}
flowchart TB
  A[VegVault] --> B[Community stream]
  A --> C[Climate stream]
  B --> B1[Counts to proportions]
  B1 --> B2[Age uncertainty to 500 yr grid]
  B2 --> B3[GBIF + auxiliary taxonomy]
  B3 --> B4[Plantae and configured resolution]
  B4 --> B5[Rare/core/sample filters]
  C --> C1[CHELSA-TraCE21k BIO variables]
  C1 --> C2[Drop zero-variance predictors]
  C2 --> C3[Collinearity screening]
  C3 --> C4[Selected predictors on 500 yr grid]
  B5 --> D[Model input]
  C4 --> D
```
