...
source(
  here::here(
    "Documentation/Presentations/IAVS_2026/R/Visualisation/Extra/functional_type_nmds_figure.R"
  )
)
```
...
knitr::include_graphics(
  here::here(
    "Documentation/Presentations/IAVS_2026/figures/results/extra/slide_extra_05_functional_type_nmds.png"
  )
)
```
...
```{r}
#| label: extra-pipeline-spatial-generate
#| include: false
#| cache: true
source(
  here::here(
    "Documentation/Presentations/IAVS_2026/R/Visualisation/pipeline_mermaid_spatial.R"
  )
)
```

```{r}
#| label: extra-pipeline-spatial-render
#| results: asis
#| echo: false
#| cache: false
path_mmd <- here::here(
...
if (fs::file_exists(path_mmd)) {
  cat("```{mermaid}\n")
  cat(base::readLines(path_mmd), sep = "\n")
  cat("\n```\n")
}
```
...
```{r}
#| label: extra-pipeline-temporal-generate
#| include: false
#| cache: true
source(
  here::here(
    "Documentation/Presentations/IAVS_2026/R/Visualisation/pipeline_mermaid_temporal.R"
  )
)
```

```{r}
#| label: extra-pipeline-temporal-render
#| results: asis
#| echo: false
#| cache: false
path_mmd <- here::here(
