...
read_target_or_null <- function(target_name, store_path) {
  purrr::possibly(
    ...
    otherwise = NULL
  )(target_name, store_path)
}

read_target_meta_or_empty <- function(store_path) {
  purrr::possibly(
    .f = function(store) {
      targets::tar_meta(
        fields = c("name", "error"),
...
count_successful_targets <- function(store_path, target_pattern) {
  data_meta <-
...
number_trait_values <-
  format_slide_count(base::nrow(data_traits_classified_corrected))
...
number_of_functional_types <-
  format_slide_count(number_of_functional_types_raw)
```
