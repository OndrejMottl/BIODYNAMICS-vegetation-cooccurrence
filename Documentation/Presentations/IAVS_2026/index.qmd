...
::: {.column width="60%" .fragment .tui-reveal-blur data-fragment-index="2" .image-height-600}

<!--- 

Lets actually not use the R script and figure as it is very hard to manually layout the steps. The code is there but silenced. 
...
flowchart TB
  source["VegVault<br/>records"]

  subgraph community_stream [COMMUNITY STREAM]
    direction TB
    community_counts["counts to proportions"]
    community_age["age uncertainty<br/>500 yr grid"]
    community_taxa["GBIF + aux taxonomy"]
    community_filters["Plantae + resolution<br/>rare/core/sample filters"]
    community_counts --> community_age --> community_taxa --> community_filters
  end

  subgraph climate_stream [CLIMATE STREAM]
    direction TB
    climate_vars["CHELSA BIO candidates"]
    climate_variance["drop zero-variance vars"]
    climate_screen["select non-collinear set"]
    climate_grid["interpolate to 500 yr grid"]
  ...
  source --> climate_vars
  community_filters --> model
  climate_grid --> model

  class source source_node
...
knitr::include_graphics(
  here::here(
    "Documentation/Presentations/IAVS_2026/figures/results/panels/slide_08_spatial_units_map.png"
  )
)
```
...
knitr::include_graphics(
  here::here(
    "Documentation/Presentations/IAVS_2026/figures/results/panels/slide_08_spatial_tiles.png"
  )
)
```
