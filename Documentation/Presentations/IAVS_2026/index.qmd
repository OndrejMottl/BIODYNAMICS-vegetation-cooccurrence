...
::: {.oracle-line}
I am [[O]]{.oracle-highlight .oracle-highlight-purple}bservational [[R]]{.oracle-highlight .oracle-highlight-purple}untime for [[A]]{.oracle-highlight .oracle-highlight-purple}nalysis of [[C]]{.oracle-highlight .oracle-highlight-purple}ommunity-[[L]]{.oracle-highlight .oracle-highlight-purple}evel [[E]]{.oracle-highlight .oracle-highlight-purple}cology**
:::

::: {.oracle-line}
[&lt;ORACLE&gt;]{.oracle-highlight .oracle-highlight-purple} for short.
:::
...
<!-- slide:02 -->
## Is there a [scale dependence]{.text-color-temporal .text-bold} in the amount of unexplained variation ([potentially due to biotic interactions]{.text-color-latent}) structuring vegetation? {.slide-terminal .slide-terminal-question}
...
::: {.oracle-line}
 - residual species-species association.
:::
:::
...
<!-- slide:03 -->
## Archive: [&lt;VegVault&gt;]{.text-color-cyan} {.slide-terminal .slide-terminal-methods}

:::: {.columns}
::: {.column width="62%" }

::: {.image-height-400 .fragment .tui-reveal data-fragment-index="2"}
--Northern Hemisphere coverage figure--
:::

::: {.image-height-400 .fragment .tui-reveal data-fragment-index="3"}
--figure/schematics of data input--
:::
:::

::: {.column width="38%" }
::: {.oracle-terminal .fragment .tui-reveal data-fragment-index="1"}

::: {.oracle-line}
&lt;VegVault&gt; database: publicly available oracle-line open source database
:::

::: {.oracle-line}
VegVault scan complete.
:::
...
::: {.oracle-line}
Due to data availability, I will focus on the Northern Hemisphere of the planet.
:::
:::

::: {.fragment .tui-reveal data-fragment-index="4"}
...
::: {.oracle-line}
 - Spatial-resolution runs : chnages with spatial scale
:::
...
::: {.oracle-line}
- Taxonomic aggregation level : change through classification.
:::
:::
...
::: {.oracle-line}
 - Taxa are classified automatically to GBIF and filtered.
:::
...
::: {.column width="60%" .fragment .tui-reveal data-fragment-index="2" .image-height-750}
...
:::: {.columns}
::: {.column width="65%" .fragment .tui-reveal data-fragment-index="2" .image-height-750}
...
::: {.oracle-line}
[Abiotic predictors]{.oracle-highlight .oracle-highlight-phosphor} explain shared response.
:::

[]{.oracle-line}

::: {.oracle-line}
[Moran Eigenvector Maps]{.oracle-highlight .oracle-highlight-spatial} (MEMs) absorb spatio-temporal autocorrelation.
:::

[]{.oracle-line}

::: {.oracle-line}
[Residual covariance]{.oracle-highlight .oracle-highlight-latent} carries species-species association signal.
:::
...
::: {.column width="50%" .fragment .tui-reveal data-fragment-index="1"}
...
::: {.oracle-line}
[Caution]{.oracle-highlight .oracle-highlight-red}: co-occurrence is not proof of [interaction]{.oracle-highlight}.
:::

:::
:::

::: {.column width="50%" .fragment .tui-reveal data-fragment-index="2" .image-height-750} 
...
<!-- slide:08 -->
## Spatial results: local to continental {.slide-terminal .slide-terminal-results}

:::: {.columns}
::: {.column width="50%"}

::: {.image-height-600 .fragment .tui-reveal data-fragment-index="2"}
...
::: {.column width="50%"}
::: {.image-height-600 .fragment .tui-reveal data-fragment-index="3"}
...
<!-- slide:09 -->
## How does the spatial pattern change when we add taxonomic resolution? {.slide-terminal .slide-terminal-question}
...
<!-- slide:10 -->
## Is the association signal stable through time? {.slide-terminal .slide-terminal-question}
...
:::: {.columns}
::: {.column width="33.33%" .fragment .tui-reveal  data-fragment-index="1" .image-height-600}
--NORTH AMERICA--
:::

::: {.column width="33.33%" .fragment .tui-reveal  data-fragment-index="2" .image-height-600}
--EUROPE--
:::

::: {.column width="33.33%" .fragment .tui-reveal  data-fragment-index="3" .image-height-600}
--ASIA--
:::
...
<!-- slide:13 -->
## Future predictions {.slide-terminal .slide-terminal-synthesis}
...
::: {.oracle-line}
Current models can be used to futher predict the biodiversity patterns under the climate change scenarios.
:::
:::
...
<!-- slide:14 -->
## Query resolved ... termination {.slide-terminal .slide-terminal-story}
...
::: {.column width="35%" .fragment .tui-reveal data-fragment-index="2"}
...
Interested in [macroecology]{.text-color-amber}, [palaeoecology]{.text-color-cyan}, [biodiversity]{.text-color-purple}, and [data science]{.text-color-red}
...
[This presentation is publicly available on BIODYNAMICS project websites]{.display-block .text-weight-heading .text-line-height-title .margin-bottom-large .text-shadow-dim .text-uppercase .text-size-heading .text-color-purple}
...
::: {.text-color-muted .text-letter-spacing-status .text-uppercase .text-bold .r-fit-text .text-underline}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function initTypewriter() {
    const lines = document.querySelectorAll('.oracle-line');

    function resolveTypewriterSlide(target) {
      if (!target) return null;

      if (target.matches && target.matches('section')) {
        return target;
      }

      return target.closest ? target.closest('section') : null;
    }
    ...
    lines.forEach((line) => {
      ...
      while (node = walk.nextNode()) {
        ...
        node.nodeValue = ''; // Clear initially
      }
    ...
    function runTypewriter(slide) {
      slide = resolveTypewriterSlide(slide);

      if (!slide) return;
      const slideLines = slide.querySelectorAll('.oracle-line');
      let cursor = slide.typewriterCursor;
      const runId = (slide.typewriterRunId || 0) + 1;
      slide.typewriterRunId = runId;

      if (!cursor) {
        cursor = document.createElement('span');
        cursor.className = 'oracle-cursor';
        slide.typewriterCursor = cursor;
      }
      ...
      slideLines.forEach((line) => {
        ...
        if (line.typingTimeout) {
          clearTimeout(line.typingTimeout);
        }

        line.classList.remove('oracle-line-prompt-visible');

        data.textNodes.forEach(item => {
          item.node.nodeValue = '';
        });
        data.currentNodeIndex = 0;
        data.currentCharIndex = 0;
      });

      function typeLine(lineIndex) {
        if (slide.typewriterRunId !== runId) return;

        if (lineIndex >= slideLines.length) {
          cursor.classList.add('finished');
          return;
        }

        const line = slideLines[lineIndex];
        const data = line.typewriterData;
        ...
        function typeNextChar() {
          ...
          if (data.currentNodeIndex < data.textNodes.length) {
            ...
            if (data.currentCharIndex < text.length) {
              ...
              line.typingTimeout = setTimeout(typeNextChar, 18); // Rapid phosphor terminal draw speed!
            } else {
    ...
    // Run on initial slide
    runTypewriter(Reveal.getCurrentSlide());
    
    // Listen to slide changes
    Reveal.on('slidechanged', event => {
      runTypewriter(event.currentSlide);
    });
    
    // Listen to fragment reveals
    Reveal.on('fragmentshown', event => {
      runTypewriter(event.fragment);
    });
  }
