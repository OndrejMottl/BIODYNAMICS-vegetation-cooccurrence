...
::: {.notes}
TIME 00:25. Hello and welcome! I would like to try something different. I think that science should be serious but also a little bit fun. So I decided to do something different. I will ask the audience to close their eyes and imagine they are on a spaceship and their goal is to examine the vegetation patterns of this planet called "Earth". We will use the spaceship computer called ORACLE to do so. All the data, analysis, and results are real. The computer is not real and only serves as a narrative device.
:::
...
As a scientist, I am interested in understanding the processes that structure vegetation patterns on Earth. We know that climate and spatial factors play a big role, but I am particularly interested in the unexplained variation, which could potentially be due to biotic interactions. These interactions can take the form of shared pollinator or herbivore pressure, or direct facilitation or competition between plants.
...
::: {.notes}
TIME 00:45. Oracle, I would like to explore vegetation patterns across space and time on this planet. From our knowledge banks we know that vegetation is mostly explained by climate and spatial factors. However, I am particularly interested in the unexplained variation. My first question is simple: Does the unexplained part of the community pattern change with scale, or does it stay broadly consistent?
:::
...
::: {.notes}
TIME 00:45. Oracle, build a model that includes climate, spatial structure, and whatever residual association remains.
We will be using the {sjSDM} R package because the community matrix and spatial design are too large for slower MCMC workflows.
:::
...
:::{.notes}
Now let's focus on the first axis of the analysis, which is spatial scale.
:::
...
What do we see? Is there a clear change in the association signal across spatial scales? I would expect local units to have a stronger association signal than continental units, but the pattern is not showing!
:::
...
::: {.notes}
TIME 00:45. Oracle, how does the spatial pattern change when we add taxonomic resolution? We assume that higher taxonomic resolution will result in a smaller association signal in local units and vice versa.
Interestingly, the change in taxonomic resolution does not seem to affect the spatial patterns. This is surprising, as we would expect that higher taxonomic resolution would result in a smaller association signal in local units and vice versa.
:::
...
::: {.notes}
TIME 00:50. Oracle, now let's switch to temporal mode and test whether the association signal stays stable through time.
Now each time slice receives an independent analysis and diagnostic workflow. We will look at the temporal trajectories of the association signal, but also at the network structure, which can change even when variance components look similar. Proceed ORACLE!
:::
...
::: {.notes}
TIME 00:50. The network structure and the variance components are changing through time. There is no clear pattern change during the LGM transition or during the strong anthropogenic changes of the late Holocene. However, this is not consistent for all continents. Moreover, the association signal is not responding in the same way as the network structure.
:::
...
::: {.notes}
TIME 00:25. This presentation is publicly available. The rendered presentation and other materials are accessible via the project website. The QR code will link to the access page where slides, code, and documentation can be found.
:::
