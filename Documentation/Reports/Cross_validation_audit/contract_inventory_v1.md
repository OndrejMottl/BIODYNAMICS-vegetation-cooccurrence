...
| Repeated assignment | Assignment is deterministic for the configured seed and repeats represent distinct intended origin perturbations. | Assignment functions and deterministic tests. | Deterministic; origin coverage remains open as CV-002. |
| Candidate comparison | Compatible candidates are evaluated on the same repeat/fold evidence and selected deterministically under the documented loss/weighting rule. | Tuning summaries, tier aggregation, selection tests. | Evidence completeness stabilized; score seed and loss estimand remain open as CV-003/CV-012. |
...
| Reproducibility | Assignment and fit seeds derive from the configured seed and are recorded without depending on ambient R RNG state. | Assignment and fit runners/provenance tests. | Assignment/fit covered; stochastic scoring seed remains open as CV-003. |
