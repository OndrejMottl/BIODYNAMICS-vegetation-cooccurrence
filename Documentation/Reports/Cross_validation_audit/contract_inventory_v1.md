...
| Scientific-performance decision | Technical validity, held-out predictive skill, and calibration are classified separately under a versioned policy. Tjur R2 is discrimination, not percent variance explained. | `assess_sjsdm_scientific_performance()`, isolated scientific-reference targets, tests, and `scientific_performance_decision_contract_v1.md`. | Covered by policy `sjsdm_scientific_performance_v1`; the reference passes prediction with a calibration caution. |
...
| Scientific-performance criteria | One row per technical or scientific criterion with criterion identifier, domain, observed value, threshold, comparison direction, required flag, pass flag, and evaluability status. |
| Scientific-performance decision | One row containing policy version, technical CV status, scientific prediction status, calibration status, and semicolon-delimited failure reasons when applicable. |
...
| Scientific performance | Technical CV is `pass` or `fail`; prediction is `pass`, `fail_null_skill`, `fail_discrimination`, or `insufficient_evidence`; calibration is `acceptable`, `caution`, or `not_evaluable`. |
